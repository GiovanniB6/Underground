...
function cRP.initSystem()
	...
	if consult[1] then
		for k,v in pairs(consult) do
			table.insert(characterList,{ user_id = v["id"], name = v["name"].." "..v["name2"], date = v["date"], locate = v["locate"] })
		end
	end
...
-----------------------------------------------------------------------------------------------------------------------------------------
local charActived = {}
function cRP.newCharacter(name,name2,sex,date,locate)
	local source = source
	if charActived[source] == nil then
		...
		if sex == "mp_m_freemode_01" then
			vRP.execute("characters/newCharacter",{ steam = steam, name = name, name2 = name2, date = date, locate = locate, sex = "M", phone = vRP.generatePhone(), serial = vRP.generateSerial(), blood = math.random(4) })
		else
			vRP.execute("characters/newCharacter",{ steam = steam, name = name, name2 = name2, date = date, locate = locate, sex = "F", phone = vRP.generatePhone(), serial = vRP.generateSerial(), blood = math.random(4) })
		end
